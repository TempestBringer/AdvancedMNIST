...
from net.SampleNet import SampleNet
...
if __name__ == "__main__":
    ...
    # 实例化网络
    net = SampleNet(output_class=global_args.output_class, is_training=True)
    # 是否继续训练
    ...
    for i in range(global_args.epoch):
        ...
        for j, cur_data in tqdm(enumerate(train_data_provider, 0)):
            ...
            # print(label.shape)
            cur_loss = criterion(cur_output, label.squeeze(0))
        ...
        for j, cur_test_data in tqdm(enumerate(test_data_provider, 0)):
            ...
            total_try += 1
            test_report[i][test_label * 2] += 1
            if test_output == test_label:
                success_try += 1
                test_report[i][test_label * 2 + 1] += 1
